...
public class ViolationTypesController : Controller
{
    ...
    public async Task<IActionResult> Index(string ViolationTypeName = "", SortState sortOrder = SortState.No,
        int page = 1)
    {
        // Если параметры фильтрации пустые, считываем их из сессии
        if (string.IsNullOrEmpty(ViolationTypeName))
        {
            var sessionData = SessionExtensions.Get(HttpContext.Session, "ViolationType");
            if (sessionData != null)
            {
    ...
    public async Task<IActionResult> Details(int? id)
    {
        ...
        if (violationType == null) return NotFound();
    ...
    public async Task<IActionResult> Create(
        [Bind("ViolationTypeId,Name,PenaltyAmount,CorrectionPeriodDays")] ViolationType violationType)
    {
    ...
    public async Task<IActionResult> Edit(int id,
        [Bind("ViolationTypeId,Name,PenaltyAmount,CorrectionPeriodDays")] ViolationType violationType)
    {
        if (id != violationType.ViolationTypeId) return NotFound();
    ...
    {
        var violationType = await _context.ViolationTypes.FindAsync(id);
        if (violationType != null) _context.ViolationTypes.Remove(violationType);
    ...
    private static IQueryable<ViolationType> Sort_Search(IQueryable<ViolationType> violationTypes, SortState sortOrder,
        string searchviolationTypenName)
    {
