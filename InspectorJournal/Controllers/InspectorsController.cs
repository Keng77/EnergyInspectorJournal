...
public class InspectorsController : Controller
{
    ...
    public IActionResult Index(string InspectorName = "", SortState sortOrder = SortState.No, int page = 1)
    {
        ...
        {
            var sessionData = SessionExtensions.Get(HttpContext.Session, "Inspector");
            if (sessionData != null)
            {
    ...
    {
        if (id != inspector.InspectorId) return NotFound();
    ...
    private static IQueryable<Inspector> Sort_Search(IQueryable<Inspector> inspectors, SortState sortOrder,
